...
-(BOOL) queryConfigurationAndRegister{
	BOOL on3G = ([NgnEngine getInstance].networkService.networkType & NetworkType_WWAN);
	...
	if(on3G && !use3G){
		...
		return [[NgnEngine getInstance].sipService registerIdentity];
	}
}
...
- (void)applicationDidReceiveMemoryWarning:(UIApplication *)application {
    [[NgnEngine getInstance].contactService unload];
	[[NgnEngine getInstance].historyService clear];
	[[NgnEngine getInstance].storageService clearFavorites];
}
