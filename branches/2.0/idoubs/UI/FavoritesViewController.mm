...
-(void) refreshData{
	@synchronized(self->favorites){
		NSArray* favorites_ = [[[[NgnEngine getInstance].storageService favorites] allValues] sortedArrayUsingSelector:@selector(compareFavoriteByDisplayName:)];
...
-(void) onFavoritesEvent:(NSNotification*)notification{
	...
	switch (eargs.eventType) {
		...
		{
			NgnFavorite* favorite = [[[NgnEngine getInstance].storageService favorites] objectForKey: [NSNumber numberWithLongLong: eargs.favoriteId]];
			if(favorite){
...
- (void)tableView:(UITableView *)aTableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
	if (editingStyle == UITableViewCellEditingStyleDelete){
        ...
        if (favorite) {
			[[NgnEngine getInstance].storageService deleteFavorite:favorite];
		}
    }
}
