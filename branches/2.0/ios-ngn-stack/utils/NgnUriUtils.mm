...
+(NSString*) getDisplayName:(NSString*)uri{
	...
	if(![NgnStringUtils isNullOrEmpty: uri]){
		NgnContact* contact =  [[NgnEngine getInstance].contactService getContactByUri: uri];
		...
		if(_sipUri && _sipUri->isValid()){
			...
			contact = [[NgnEngine getInstance].contactService getContactByPhoneNumber: displayname];
...
+(NSString*)makeValidSipUri: (NSString*)uri{
	...
	if([uri hasPrefix: @"sip:"] || [uri hasPrefix: @"sips:"]){
		...
		if([NgnStringUtils contains: uri subString: @"@"]){
			...
			NSString* realm = [[NgnEngine getInstance].configurationService getStringWithKey: NETWORK_REALM];
