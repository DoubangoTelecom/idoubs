...
-(void) refreshData{
	@synchronized(self->messages){
		...
		NSArray* events = [[[[NgnEngine sharedInstance].historyService events] allValues] retain];
...
- (void)tableView:(UITableView *)aTableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
	if (editingStyle == UITableViewCellEditingStyleDelete){
        ...
        if (entry) {
			[[NgnEngine sharedInstance].historyService deleteEvents:MediaType_SMS withRemoteParty:entry.remoteParty];
		}
    }
}
